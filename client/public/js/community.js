...
const CAT_COLORS = {
  'For You':        ['#7DD8D8','#ACE6E6','#40CECE'],
  'Hollywood':      ['#A8D5BA','#D1E9DB','#73C393'],
  'K-Drama':        ['#F5C6D0','#F9DCE2','#EE869C'],
  'Anime':          ['#AEB3BA','#CFD2D6','#828E9F'],
  'Sitcoms':        ['#F0DFA0','#F8F1D5','#EBCD5E'],
  'C-Drama':        ['#C9B8E8','#E8E0F5','#9F7EDB'],
  'J-Drama':        ['#A9C9EE','#DCE9F8','#69A3E7'],
  'Thai Drama':     ['#F7C9AE','#FBE6DA','#F59D69'],
  'Turkish Dizi':   ['#C99191','#DDBABA','#B75B5B'],
  'Web Series':     ['#E3D3B8','#F3EDE2','#D3B481']
};
const DEFAULT_THEME = CAT_COLORS['For You'];

/* Applies a category's pastel palette page-wide: buttons, links, the topbar,
   and the floating dock all read these three CSS variables. Call with no
   argument (or an unrecognized category) to fall back to the default cyan theme. */
function applyPageTheme(category){
  const [accent, accentTitle, accentDeep] = CAT_COLORS[category] || DEFAULT_THEME;
  const root = document.documentElement.style;
  root.setProperty('--gold', accent);
  root.setProperty('--crimson-bright', accentTitle);
  root.setProperty('--violet', accentDeep);
}
...
function renderCommunity(){
  ...
  const [c1,c2,c3] = CAT_COLORS[activeCategory] || DEFAULT_THEME;
  section.style.setProperty('--cat-1', c1);
  section.style.setProperty('--cat-2', c2);
  section.style.setProperty('--cat-3', c3);
  applyPageTheme(activeCategory);
  ...
  el.innerHTML = list.map(p=>{
    ...
    const [pc1, pc2, pc3] = CAT_COLORS[p.category] || DEFAULT_THEME;
    return `
    <div class="post-card" style="--cat-1:${pc1}; --cat-2:${pc2}; --cat-3:${pc3};">
      <div class="post-head">
